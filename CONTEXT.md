...
- **Soft Commitment**: An `Event` scheduled by Planaro that the algorithm is allowed to re-schedule if better `Slots` become available or `Hard Commitments` conflict with it.
- **Planaro Calendar**: A dedicated secondary calendar in the user's Google account used exclusively for `Soft Commitments`.
- **Primary Calendar**: The user's main Google Calendar where `Hard Commitments` (appointments, meetings, etc.) are typically stored.
...
- **Staging Area**: A UI component containing a list of `Tasks` that have not yet been converted into `Soft Commitments`.
