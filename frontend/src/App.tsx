...
function App() {
  ...
  const handleSync = async () => {
    ...
    setSyncing(true);
    try {
      const planaroCal = await getOrCreatePlanaroCalendar(
      ...
      const planaroEvents = await fetchEvents(
        session.provider_token,
        planaroCal.id,
        timeMin,
      );

      const taggedPlanaroEvents = planaroEvents.map((e) => ({
        ...e,
        isSoft: true,
      }));

      setEvents(
        [...primaryEvents, ...taggedPlanaroEvents].sort((a, b) => {
        ...
        {session ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center max-w-md mx-auto">
            ...
            <p className="text-muted-foreground mb-8">
              Planaro is your AI-powered scheduling assistant that works with
